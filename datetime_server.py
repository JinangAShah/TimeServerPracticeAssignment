...
def age_calc():
    ...
    x = datetime.date(datetime(int(in_date[2]), int(in_date[0]), int(in_date[1])))
...
def time_until_meal(meal):
    ...
    datetime1 = (datetime.combine(datetime.date
                                  (datetime(year=c.year, month=c.month, day=c.day)), present_time))
    datetime2 = (datetime.combine(datetime.date
                                  (datetime(year=c.year, month=c.month, day=c.day)), meal_time))
    time_elapsed = datetime2 - datetime1
    meal_time_in = (time_elapsed + timedelta(hours=0, minutes=1440, seconds=0))
    ...
    if seconds / 60 > 0:
        minutes = minutes + 1
    print(hours, minutes)
    return 'Time until next {}: {} hours'.format(meal,
                                                 hours + math.ceil(minutes) * 0.01)
